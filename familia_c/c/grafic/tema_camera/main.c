...
float x = 0, y = 0, delta = 0.1;
void CALLBACK procX0(void)
{
	x -= delta;
	printf("\n %s %f", "X:", x);
}
void CALLBACK procX1(void)
{
	x += delta;
	printf("\n %s %f", "X:", x);
}
void CALLBACK procY0(void)
{
	y -= delta;
	printf("\n %s %f", "Y:", y);
}
void CALLBACK procY1(void)
{
	y += delta;
	printf("\n %s %f", "Y:", y);
}
void CALLBACK reset(void)
{
	x = y = 0;
}
...
void CALLBACK display(void)
{
	...
	glPushMatrix();
	gluLookAt(0, 0, 1, x, y, 0, 0, 1, 0);//modifica matricea de vizualizare!
	glPushMatrix();
