...
def test4(): #testare preluare date sub forma json
    web_data=requests.get("https://httpbin.org/get")
    #.json() returneaza un dictionar,nu string
    temp=json.dumps(web_data.json())#formatare ca string
    file_buffer=json.dumps(web_data.json()).encode("utf-8")#formatare ca octeti
    print(file_buffer)
